...
func (hl *HiddenLayerStruct) ForwardHiddenLayer(X t.Tensor) t.Tensor {
	hl.Layer.Foward(X)
	hl.Activation.Forward(hl.Layer.Output)
	return hl.Activation.Output
}

func (hl *HiddenLayerStruct) BackwardHiddenLayer(X t.Tensor) t.Tensor {
...
func (oL *OutputLayerStruct) ForwardOutputLayer(X t.Tensor) t.Tensor {
	oL.Layer.Foward(X)
	oL.Activation.Forward(oL.Layer.Output)
	return oL.Activation.Outpout
}
func (oL *OutputLayerStruct) BackwardOutputLayer(y t.Tensor) t.Tensor {
...
func (nn *NeuralNetwork) Fit(X, y, xTest, yTest t.Tensor, batch int) {
	...
	for i := 0; i < nn.epoch; i++ {
		...
		for _, hLayer := range nn.HiddenLayer {
			input = hLayer.ForwardHiddenLayer(input)
		}
		pred := nn.OutPutLayer.ForwardOutputLayer(input)
		backWardOutput := nn.OutPutLayer.BackwardOutputLayer(yNew)
		for i := len(nn.HiddenLayer) - 1; i >= 0; i-- {
			if i == -1 {
				break
			}
			backWardOutput = nn.HiddenLayer[i].BackwardHiddenLayer(backWardOutput)
		}
		inputTest := xTest
		for _, hLayer := range nn.HiddenLayer {
			inputTest = hLayer.ForwardHiddenLayer(inputTest)
		}
		predTest := nn.OutPutLayer.ForwardOutputLayer(inputTest)
